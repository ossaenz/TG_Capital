  ...
  function buildSchwabViewHTML() {
    return `
      <style>
        ...
        .sg-journal-card { margin-top: 10px; border: 1px solid var(--border); border-radius: 8px; padding: 10px; background: var(--bg2); }
        .sg-journal-title { font-size: 10px; text-transform: uppercase; letter-spacing: .08em; color: var(--blue, #2563eb); margin-bottom: 8px; font-weight: 700; }
        .sg-journal-meta { font-size: 11px; color: var(--text2); margin-bottom: 6px; }
        .sg-journal-notes { font-size: 12px; color: var(--text0); margin-bottom: 8px; }
        .sg-journal-status { font-size: 11px; color: var(--text2); }
  ...
  function wireSchwabView(view) {
    ...
    window.sgSendChat = async function () {
      ...
      try {
        ...
        const aiEl = addMsg('ai', reply);
        addJournalSuggestion(aiEl, r.suggestedJournal);
    ...
    function addJournalSuggestion(parent, draft) {
      if (!parent || !draft?.symbol || !draft?.notes) return;
      const card = document.createElement('div');
      card.className = 'sg-journal-card';
      const title = document.createElement('div');
      title.className = 'sg-journal-title';
      title.textContent = 'Suggested Journal Entry';
      const meta = document.createElement('div');
      meta.className = 'sg-journal-meta';
      meta.textContent = `${draft.date || 'No date'} · ${draft.symbol}${draft.action ? ' · ' + draft.action : ''}${draft.strategy ? ' · ' + draft.strategy : ''}`;
      const notes = document.createElement('div');
      notes.className = 'sg-journal-notes';
      notes.textContent = draft.notes;
      const btn = document.createElement('button');
      btn.className = 'sg-btn sg-btn-primary';
      btn.style.padding = '6px 10px';
      btn.style.fontSize = '11px';
      btn.textContent = 'Approve And Save';
      const status = document.createElement('div');
      status.className = 'sg-journal-status';
      status.textContent = 'Review the draft before saving.';
      btn.onclick = async function () {
        ...
        try {
          ...
          if (!r.ok || r.error) throw new Error(r.error || 'Save failed');
          btn.textContent = 'Saved';
          status.textContent = `Saved ${draft.symbol} journal entry for ${draft.date}.`;
        } catch (e) {
          btn.disabled = false;
