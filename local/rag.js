...
function journalEntryToText(e) {
  const parts = [`Journal [${e.date_iso || e.date || '?'}]${e.source === 'auto' ? ' (auto-generated technical note)' : ''}`];
  ...
  if (e.objective) parts.push(`Objective: ${e.objective}`);
  if (e.thesis)    parts.push(`Rationale: ${e.thesis}`);
  if (e.notes)     parts.push(`Notes: ${e.notes}`);
  if (e.market_iv_rank != null || e.market_support != null) {
    parts.push(`Market at execution: IV Rank ~${e.market_iv_rank ?? '?'}/100 (proxy), support $${e.market_support ?? '?'}, resistance $${e.market_resistance ?? '?'}`);
  }
  return parts.join('\n');
}
...
// ── Freeform notes (source='note' in rag_docs) ─────────────────────────────────
// Lets the chat model itself persist arbitrary knowledge on request ("remember
// this", "add this to your memory") through the same embedding/search pipeline
// used for trades/journal — no separate table or retrieval path needed since
// rag_docs is already a generic (source, ref_id, content, metadata, embedding)
// store. Embedding failure is non-fatal (falls back to the keyword search in
// search() below), same as indexAll()'s per-doc try/catch.
async function saveNote(db, { title, content, tags }, ollamaHost, embedModel) {
  const refId = `${Date.now()}-${(title || 'note').toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, '').slice(0, 60)}`;
  const text = `Note${title ? `: ${title}` : ''}${tags ? ` [${tags}]` : ''}\n${content}`;

  let embedding = null;
  try { embedding = JSON.stringify(await embed(text, ollamaHost, embedModel)); } catch {}

  db.prepare(`
    INSERT OR REPLACE INTO rag_docs (source, ref_id, content, metadata, embedding)
    VALUES ('note', ?, ?, ?, ?)
  `).run(refId, text, JSON.stringify({ title: title || null, tags: tags || null, savedAt: new Date().toISOString() }), embedding);

  return { refId, embedded: !!embedding };
}
...
module.exports = { initRag, indexAll, saveJournalEntries, saveNote, search, buildContext, getStatus, getIndexState };
